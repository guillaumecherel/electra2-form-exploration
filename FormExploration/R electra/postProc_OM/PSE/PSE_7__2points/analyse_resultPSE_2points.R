...
dir = "data/resultsPSE_7_2/"
files = list.files(dir)
nbFiles = length(files)

# number of points
generation = seq(1000,200*(nbFiles), by = 1000)
...
source("utils_kohonen_on_PSE_2points.R")

## import data
resPSE = read.csv("data/resultsPSE_7_2/population20400.csv")
dim(resPSE)
...
df %>% group_by(nbPointsSinguliersF) %>% summarise(count = n())
# courbure 
hist(df$courbureMoyenneAbsolueD)
hist(df$courbureMoyenneAbsolueF)
# indice motif
hist(df$indiceMotifD)
hist(df$indiceMotifF)


df2 = df %>% select(nbPointsSinguliersD,courbureMoyenneAbsolueD,indiceMotifD,nbPointsSinguliersF,courbureMoyenneAbsolueF,indiceMotifF)
df2 = as.matrix(df2)
df2 = scale(df2)
dim(df2)
...
ggsave("tableauPlot_kohonen_PSE_7_2_twoPoint_shortTime.png",pp, width = 15, height = 15, dpi = 400) #dpi = 72)
