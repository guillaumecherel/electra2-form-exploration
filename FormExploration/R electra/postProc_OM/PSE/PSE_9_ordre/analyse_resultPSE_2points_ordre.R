...
options(scipen=999) # pour eviter ecriture scientifique
for (i in generation){
  temp <- read.csv(paste0(dir,"population",i,".csv"), header = T)
  sampleSize = c(sampleSize,dim(temp)[1])
  df = as.data.frame(temp)
  #fullSize=dim(resPSE)[1]
  df = df %>%  filter(s1 <=5,s2 <=5, c1 <=50, c2 <=50, m1<=2000, m2<=2000)
  reduceSize= c(reduceSize,dim(df)[1]) 
}

generation = c(0,generation)
sampleSize = c(0,sampleSize)
reduceSize = c(0,reduceSize)
...
source("utils_kohonen_on_PSE_2points_ordre.R")

## import data
resPSE = read.csv("data/resultsPSE_9_1/population1400.csv")
dim(resPSE)
...
df = df %>%  filter(v2_PSE < -0.2 | v2_PSE > 0.2, v3_PSE < -0.2 | v3_PSE > 0.2)  
dim(df)



df2 = df %>% select(s1,c1,m1,s2,c2,m2)
df2 = as.matrix(df2)
df2 = scale(df2)
dim(df2)
...
carte <-som(df2,grid=somgrid(7,7,"rectangular"), rlen=5000)
carte <-som(df2,grid=somgrid(9,8,"hexagonal"), rlen=5000)
...
plot_traj_representantNode2(resRepr,times)
...
dirRes = "resultsPlotKohonen_twoPoint"
#dir.create(dirRes)
...
liste_plots = lapply(1:nbPointsGrille, function(i){createPlot2(i,carte,df2,df,nbMotifToPlot) })
#liste_plots[[1]]
...
pp = reduce(liste_plots, function(a,b){a+b}) + plot_layout(ncol = 7)
...
ggsave(paste0("tableauPlot_kohonen_PSE_9_1_twoPoint_ordre_T=",Tfinal,"sansVitesse.png"),pp, width = 15, height = 15, dpi = 400) #dpi = 72)
ggsave(paste0("tableauPlot_kohonen_PSE_9_1_twoPoint_ordre_shortTime_nbMot=",nbMotifToPlot,"Taille_grille=",tailleGrid,".png"),pp, width = 15, height = 15, dpi = 400) #dpi = 72)

# good print
ggsave(paste0("tableauPlot_kohonen_PSE_9_1_twoPoint_ordre_T=",Tfinal,"sansVitesse.png"),pp, width = 100, height = 100, dpi = 500, units = "cm") #dpi = 72)
