...
# cherche le point le plus proche parmi tout df
findClosestRepresentantInDf2 = function(i, carte, df){
  #selInd_i = which(carte$unit.classif == i)
  #length(selInd_i)
  codebokk_i = carte$codes[[1]][i,]
  ...
  resCompa  = apply(df[,], 1, function(a){distanceVectors(a,codebokk_i)})
  # le codebook n'est pas parmis les noeuds
  # on prend donc le point qui en est le plus proche
  selId_Repr = which(resCompa == min(resCompa))  # l'indice dans les points du noeud
...
plot_traj_representantNode = function(resRepr){
  ...
  lightE = F #if(resRepr$lightD_double >=0.5){T}else{F} 
  lightF = if(resRepr$lightF_double >=0.5){T}else{F}
  lightG = F #if(resRepr$lightF_double >=0.5){T}else{F}
  
  # Tfinal = 3
  # deltaT = 0.0005
  # times = seq(0,Tfinal,by = deltaT )
  
  plotTrajectorySmart2(times, 1,resRepr$v2_PSE,resRepr$v3_PSE, rB,rC,rD,rE,rF,rG,rH,rI, angleH,angleI, angleIni_B,angleIni_D,angleIni_F,lightB,lightC,lightD,lightE,lightF,lightG)  
...
plot_traj_representantNode2 = function(resRepr, times){
  lightB = F
  lightC = F
  lightD = if(resRepr$lightD_double >=0.5){T}else{F}
  lightE = F #if(resRepr$lightD_double >=0.5){T}else{F} 
...
createPlot = function(i,carte,df2,df){
  ...
  resRepr = df[findClosestRepresentantInDf2(i,carte,df2),]
...
# faire dépendre le tamps de tracé du motif
createPlot2 = function(i,carte,df2,df,nbMotifToPlot){
  resRepr = df[findClosestRepresentantInDf2(i,carte,df2),]
  deltaT = 0.001
  Tfinal = (resRepr$m2) * nbMotifToPlot * deltaT
  times = seq(0,Tfinal,by = deltaT )
  p=plot_traj_representantNode2(resRepr,times)  
  p
}
